...
ggsave("ammoniumtrialclean_1013.jpg", width = 10, height = 8)
...
gr_LL_rd1_clean <- gr_LL_rd1 %>%
  mutate(
    Transfer = 1,  # if it doesn't exist, assume 1
    ID = paste(bact1, N, Round, Rep, sep = "_")
  ) %>%
  select(bact1, N, Rep, Transfer, ID, mumax)

gr_LL_rd1_clean <- gr_LL_rd1_clean %>%
...
```
ggsave("hotonestrial_1013.jpg", width = 7, height = 6)
