...
res
```
## Growth rates for Round 4 (S.T.2) LL
```{r}
library(ggplot2)
library(dplyr)
ammonium_trial %>% filter(Round == "4", Culture == "S.T. 2 from LL 7/23" ) %>% ggplot(aes(x = Date, y = RFU, color = Culture, shape = factor (Round)) ) +
  geom_point () +
  theme_test () +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_shape_manual(values = c(16)) +
...
```{r}
library(growthrates)
rnd1 <- ammonium_trial %>% filter(Round == "4", Culture == "S.T. 2 from LL 7/23" ) %>%
  mutate(Time = as.numeric(difftime(Date,min(Date),units = "days")))
...
## Growth rates for Round 4 (S.T. 4) OTB
```{r}
library(ggplot2)
library(dplyr)
ammonium_trial %>% filter(Round == "4", Culture == "S.T. 2 from OTB 6/10" ) %>% ggplot(aes(x = Date, y = RFU, color = Culture, shape = factor (Round)) ) +
  geom_point () +
  theme_test () +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_shape_manual(values = c(16)) +
...
```{r}
library(growthrates)
rnd1 <- ammonium_trial %>% filter(Round == "4", Culture == "S.T. 2 from OTB 6/10" ) %>%
  mutate(Time = as.numeric(difftime(Date,min(Date),units = "days")))
...
```{r}
hottrial %>%
  filter(Round %in% c(2, 3, 4, 5, 6, 9, 10, 11, 12, 13)) %>%
 ...
 mutate(Round_label = factor(Round_label, levels = c(
    "Round 1 in 30°C",
    "S.T. 1",
    "S.T. 2 in 31°C",
    "S.T. 3",
...
```
ggsave("hotonestrial_0330.jpg", width = 13, height = 8)
