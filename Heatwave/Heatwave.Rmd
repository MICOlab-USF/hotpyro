...
# For df1: already has Round = 1 or 3 (NH4)
df1 <- df1 %>%
  mutate(
    Round = case_when(
      Round == 1 ~ 1,
...
df2_clean <- df2 %>%
  ...
  mutate(
    Round = case_when(
      Transfer == 1 ~ 1,
      Transfer == 2 ~ 2  # Already matches df1 Round mapping
    ),
...
ggplot(combined_df, aes(x = N, y = mumax, fill = bact1)) +
  ...
  theme_minimal(base_size = 14)
```
ggsave("NH4_NO3_boxplot.jpg", width = 7, height = 6)
