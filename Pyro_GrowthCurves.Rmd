...
## Morphology
```{r}
photos <- read.csv("~/desktop//pyro/HeatWave_Preliminary/Pyro_Photos.csv")
...
transfer1<- hw1 %>% filter(Transfer==1) %>%  ggplot(aes(x=Date, y = CHL, fill = Isolate)) +  geom_point(shape = 21, size = 2) +
  facet_grid(temp ~ NH4 + salinity) + theme_bw() + theme(panel.grid.minor.x = element_blank(), panel.grid.minor.y = element_blank()) + scale_fill_manual(values = c("lightsalmon2"  , "#1BAAE2")) +ylab("Pyrodinium Chlorophyll-a fluorescence (RFU)") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("Trial 2") + geom_hline(yintercept=1975)
...
ggsave("pyro_maxChla.png", width = 11, height = 6 )
